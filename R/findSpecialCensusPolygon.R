...
findSpecialCensusPolygon <- function(df=NULL, plot=FALSE) {
  ...
  subdivisions <- st_transform(subdivisions, 4326)
  subdivisions <- st_make_valid(subdivisions)
  ...
  #browser()
## NEW
final <- rep('intersect', nrow(output_df))
  for (i in seq_along(1:nrow(output_df))) {
    message(i)
    ...
    if (!(output_df$CSDNAME_closest[i] == output_df$CSDNAME_intersect[i])) {
      ...
      if (centroid_inside) {
        # Close is inside intersect
        distance_from_sch_to_close <- min(as.numeric(st_distance(
          df$geometry[i],
          subdivisions$geometry[
            match(output_df$CSDUID_closest[i],
                  subdivisions_centroids$CSDUID)
          ]
        )))

        if (distance_from_sch_to_close < 1000) { # 1 km
          final[[i]] <- 'close'
        }
